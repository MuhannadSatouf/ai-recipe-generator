  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          error:
            "Google API key is not configured. Please add GOOGLE_API_KEY to your .env.local file.",
        },
        { status: 500 }
      );
    }
...
`;
    ...
    if (error instanceof Error && error.message.includes("API key not valid")) {
      return NextResponse.json(
        {
          error:
            "Invalid Google API key. Please check your GOOGLE_API_KEY in .env.local file.",
        },
        { status: 500 }
      );
    }
